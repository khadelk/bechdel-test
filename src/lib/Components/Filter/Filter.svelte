...
{#if clicked}
  <div transition:slide class="filter-container">
    <div class="buttons">
      <div
        class="{$genreClicked ? 'clicked' : ''} button genre"
        on:click={() =>
          ($genreClicked = !$genreClicked) && ($bechdelClicked = false) && ($yearClicked = false)}
      >
        <span>genre</span>
      ...
      <div
        class="{$bechdelClicked ? 'clicked' : ''} button bechdel"
        on:click={() =>
          ($bechdelClicked = !$bechdelClicked) && ($genreClicked = false) && ($yearClicked = false)}
      >
      ...
      <div
        class="{$yearClicked ? 'clicked' : ''} button year"
        on:click={() =>
          ($yearClicked = !$yearClicked) && ($genreClicked = false) && ($bechdelClicked = false)}
      >
        <span> year </span>
        <svg class="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"
          ><!--! Font Awesome Pro 6.1.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path
