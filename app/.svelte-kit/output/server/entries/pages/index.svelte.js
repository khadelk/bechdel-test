import { c as create_ssr_component, a as subscribe, e as escape, b as add_attribute, d as createEventDispatcher, f as each, g as now, l as loop, v as validate_component, h as noop, i as set_store_value, n as null_to_empty } from "../../chunks/index-76a30155.js";
import { b as bechdelData, f as filteredData, m as movieData, g as genreClicked, a as filteredBechdelData, c as bechdelClicked, w as writable, y as yearClicked } from "../../chunks/stores-eeb6b843.js";
...
const MovieCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  bechdelRating = bechdelMovie[0]?.rating;
  ...
  return `${movie.poster_path != null ? `<div class="${"container svelte-ry8ir8"}"><div class="${"movie-card svelte-ry8ir8"}"><svg class="${"black-circle svelte-ry8ir8"}" xmlns="${"http://www.w3.org/2000/svg"}" viewBox="${"0 0 512 512"}"><path d="${"M512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256z"}"></path></svg>
		...
		<a href="${"/" + escape(imdb_id)}"><div class="${"color-overlay svelte-ry8ir8"}"><div class="${"text-overlay svelte-ry8ir8"}"><div class="${"movie-title svelte-ry8ir8"}">${escape(movie.title)} passes ${escape(bechdelRating)}/3 tests </div></div></div>
			<img${add_attribute("src", "https://image.tmdb.org/t/p/w300" + movie.poster_path, 0)}${add_attribute("alt", movie.title, 0)} title="${"Test"}" class="${"svelte-ry8ir8"}"></a></div></div>` : ``}`;
});
...
const GenreFilter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $movieData, $$unsubscribe_movieData;
  let $genreClicked, $$unsubscribe_genreClicked;
  $$unsubscribe_movieData = subscribe(movieData, (value) => $movieData = value);
  $$unsubscribe_genreClicked = subscribe(genreClicked, (value) => $genreClicked = value);
  let { genre } = $$props;
  let current;
  let storeGenres = [];
  ...
  return `<div class="${"genre-options"}">${$genreClicked ? `<div class="${"options svelte-4n53sp"}">${each(genres, (genre2) => {
    return `<div class="${["svelte-4n53sp", current == genre2 ? "selected" : ""].join(" ").trim()}">${escape(genre2)} </div>`;
  })}</div>` : ``}
</div>`;
});
...
const BechdelFilter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $$unsubscribe_bechdelData;
  let $$unsubscribe_filteredBechdelData;
  let $bechdelClicked, $$unsubscribe_bechdelClicked;
  $$unsubscribe_bechdelData = subscribe(bechdelData, (value) => value);
  $$unsubscribe_filteredBechdelData = subscribe(filteredBechdelData, (value) => value);
  $$unsubscribe_bechdelClicked = subscribe(bechdelClicked, (value) => $bechdelClicked = value);
  ...
  return `<div class="${"bechdel"}">${$bechdelClicked ? `<div class="${"options svelte-z2ozv2"}">${each(bechdelNums, (num) => {
    return `<div class="${["svelte-z2ozv2", rating == num ? "selected" : ""].join(" ").trim()}">${escape(num)}</div>`;
  })}</div>` : ``}
</div>`;
});
...
const YearFilter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let minYear;
  let maxYear;
  let $bechdelData, $$unsubscribe_bechdelData;
  let $yearClicked, $$unsubscribe_yearClicked;
  $$unsubscribe_bechdelData = subscribe(bechdelData, (value) => $bechdelData = value);
  $$unsubscribe_yearClicked = subscribe(yearClicked, (value) => $yearClicked = value);
  ...
  do {
    ...
    $$rendered = `<div class="${"year"}">${$yearClicked ? `<div>${validate_component(RangeSlider, "RangeSlider").$$render($$result, {
      float: true,
      range: true,
      min: minYear,
      max: maxYear,
...
const Filter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    {
      {
        if ($bechdelClicked) {
          set_store_value(yearClicked, $yearClicked = false, $yearClicked);
          set_store_value(genreClicked, $genreClicked = false, $genreClicked);
        }
        if ($yearClicked) {
          set_store_value(genreClicked, $genreClicked = false, $genreClicked);
          set_store_value(bechdelClicked, $bechdelClicked = false, $bechdelClicked);
        }
        if ($genreClicked) {
          set_store_value(bechdelClicked, $bechdelClicked = false, $bechdelClicked);
          set_store_value(yearClicked, $yearClicked = false, $yearClicked);
    ...
    $$rendered = `${clicked ? `<div class="${"filter-container svelte-1qf9u11"}"><div class="${"buttons svelte-1qf9u11"}"><div class="${escape($genreClicked ? "clicked" : "") + " button genre"}"><span>genre</span>
				...
				<div class="${escape($bechdelClicked ? "clicked" : "") + " button bechdel"}"><span>bechdel rating</span>
					<svg class="${"icon"}" xmlns="${"http://www.w3.org/2000/svg"}" viewBox="${"0 0 448 512"}"><path d="${"M224 416c-8.188 0-16.38-3.125-22.62-9.375l-192-192c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0L224 338.8l169.4-169.4c12.5-12.5 32.75-12.5 45.25 0s12.5 32.75 0 45.25l-192 192C240.4 412.9 232.2 416 224 416z"}"></path></svg></div>

				<div class="${escape($yearClicked ? "clicked" : "") + " button year"}"><span>year </span>
			...
			${validate_component(GenreFilter, "GenreFilter").$$render($$result, { genre }, {
      genre: ($$value) => {
        genre = $$value;
        $$settled = false;
      }
    }, {})}
			${validate_component(BechdelFilter, "BechdelFilter").$$render($$result, { rating }, {
      rating: ($$value) => {
        rating = $$value;
        $$settled = false;
      }
    }, {})}
			${validate_component(YearFilter, "YearFilter").$$render($$result, { valuesYear }, {
      valuesYear: ($$value) => {
        valuesYear = $$value;
        $$settled = false;
      }
...
const FilterButton = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${escape(null_to_empty(clicked ? "filter-button button clicked" : "filter-button button")) + " svelte-igjlyb"}"><span>filter</span>
...
const Pagination = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<nav class="${"svelte-sist45"}"><ul class="${"pagination svelte-sist45"}">${``}
		${each(displayPageArr, (page2) => {
    return `<li class="${escape(null_to_empty(clicked == page2 ? "clicked" : "")) + " svelte-sist45"}">${escape(page2)}</li>`;
  })}
...
const Movies = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_bechdelData = subscribe(bechdelData, (value) => value);
  $$unsubscribe_yearClicked = subscribe(yearClicked, (value) => $yearClicked = value);
  $$unsubscribe_filteredBechdelData = subscribe(filteredBechdelData, (value) => $filteredBechdelData = value);
  $$unsubscribe_bechdelClicked = subscribe(bechdelClicked, (value) => $bechdelClicked = value);
  let clicked = false;
  let genre;
  let rating;
  ...
  do {
    ...
    {
      {
        if (searchTerm && searchTerm.length > 0)
          ...
          set_store_value(filteredData, $filteredData = $movieData.filter((movie) => movie.backdrop_path && movie.imdb_id), $filteredData);
        } else {
          if (rating) {
            set_store_value(filteredData, $filteredData = $movieData.filter((movie) => {
    ...
    $$rendered = `<div class="${"container svelte-f20jtb"}"><div class="${"search-container svelte-f20jtb"}"><div class="${"search"}">${validate_component(Search, "Search").$$render($$result, { searchTerm }, {
      searchTerm: ($$value) => {
        searchTerm = $$value;
        $$settled = false;
      }
    }, {})}</div>
		${validate_component(FilterButton, "FilterButton").$$render($$result, { clicked }, {
      clicked: ($$value) => {
        clicked = $$value;
        $$settled = false;
      }
...
${validate_component(Filter, "Filter").$$render($$result, { clicked, genre, rating, valuesYear }, {
      clicked: ($$value) => {
        clicked = $$value;
        $$settled = false;
      },
...
<div class="${"movies svelte-f20jtb"}">${each(displayMovies, (movie) => {
      return `${validate_component(MovieCard, "MovieCard").$$render($$result, { movie }, {}, {})}`;
    })}</div>

${validate_component(Pagination, "Pagination").$$render($$result, { start, end }, {
      start: ($$value) => {
        start = $$value;
        $$settled = false;
      },
...
const Routes = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `${validate_component(Movies, "Movies").$$render($$result, {}, {}, {})}`;
});
